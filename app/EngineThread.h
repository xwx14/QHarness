...
namespace qh {
namespace provider { class Provider; }   // 基类（MockProvider 或 MockTwoStageProvider）
...
// 按 EngineKind 创建对应的 mock+engine 子树，自洽生命周期；
...
public:
    ...
    // 构造内按 kind 创建 mock+engine 子树并组装依赖链；postMessage 非拥有，须长于本线程
    EngineThread(QPostMessage* postMessage, std::string prompt,
                 std::string workDir, EngineKind kind, QObject* parent = nullptr);
...
private:
    ...
    // _mockProvider/_engine 用基类指针，按 kind 指向具体派生类（基类有 virtual~，析构正确）
    std::unique_ptr<qh::provider::Provider> _mockProvider;
