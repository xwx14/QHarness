...
MacBashTool::MacBashTool(std::string workDir) {
    _workDir = std::move(workDir);
    _definition._name = "bash";
    _definition._description = "在工作目录运行 macOS bash 命令。请提供 command。";
...
schema::ToolResult MacBashTool::execute(const schema::ToolCall& call) {
    ...
    return result;
#else
    result._output = "MacBashTool 仅在 macOS 编译";
    result._isError = true;
    return result;
#endif
}
