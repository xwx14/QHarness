...
| `core/tests/skeleton_compile_check.cpp` 【改】 | 加 `httplib::SSLClient` 实例化做 SSL 编译期验证 |
...
`core/provider/ProviderHttp.cpp`（占位，Task 2 填实现）：
```cpp
...
- [ ] **Step 5: 改 `core/tests/skeleton_compile_check.cpp` 加 SSL spike**
...
在 `touchSkeletons()` 末尾（`tm.setPostMessage(nullPM);` 之后）加 SSL 编译验证：
```cpp
    // SSL spike：验证 httplib::SSLClient 在 OpenSSL 4.0.1 + MSVC 下可编译链接
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
    httplib::SSLClient sslCli("api.openai.com", 443);
...
```bash
git add scripts/build_openssl.bat core/CMakeLists.txt core/provider/ProviderHttp.h core/provider/ProviderHttp.cpp core/tests/skeleton_compile_check.cpp
git commit -m "集成OpenSSL4.0.1:build_openssl.bat静态编译;core CMake加find_package(OpenSSL)+CPPHTTPLIB_OPENSSL_SUPPORT+链接;新增ProviderHttp骨架;skeleton加SSLClient编译验证spike"
```
