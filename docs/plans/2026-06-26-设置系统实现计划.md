...
> **勘误（2026-06-26，Task 1 review 后）**：`Settings`/`LlmProfile` 全部成员已改下划线前缀——`_name`/`_providerType`/`_baseUrl`/`_apiKey`/`_model`/`_temperature`/`_maxTokens`/`_profiles`/`_activeProfileName`/`_enableThinking`/`_workDir`/`_enabledTools`。本计划 Task 2-5 代码块中凡访问这些成员的无前缀写法（如 `prof->baseUrl`、`p.name`、`s.enableThinking`、`loaded.profiles`），均应替换为下划线前缀写法（`prof->_baseUrl`、`p._name`、`s._enableThinking`、`loaded._profiles`），**以 `core/schema/Settings.h` 实际声明为准**。自由函数 `providerTypeToString`/`findActiveProfile` 名字不变。
