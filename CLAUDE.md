...
`engine/{Engine,EngineReActLoop}`、`provider/{Provider,ProviderOpenAI,ProviderClaude}`、`tool/{Tool,ToolRegistry,ToolManager}`、`robot/{Robot,RobotFeishu}`、`memory/{Memory,MemoryFile}`。基类纯虚（`virtual ... = 0`）；派生类的构造函数与方法体在对应 `.cpp`（骨架阶段为 `/* TODO */` 占位），头文件仅声明。`Provider::generate` 失败约定抛 `std::runtime_error`。`core/tests/skeleton_compile_check.cpp` 强制实例化各派生类做链接期检查。
