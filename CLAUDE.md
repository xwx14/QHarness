...
QHarnesscc 是依赖 Qt 的可扩展大模型（LLM）Harness，C++17 + CMake + MSVC。它是 Go 项目 `E:\MyHarness\go-tiny-claw` 的 C++ 演进版，目标是把 Go 版的 schema/Engine/Provider/Tool/Memory/Robot 架构移植到 C++ 并加上 Qt 界面。

**当前完成度**：schema 数据结构与序列化（完整）、抽象类骨架（仅声明）、Qt 主窗口（最小可运行）。Engine/Provider/Tool/Memory/Robot 的派生类方法体仍是 `/* TODO */` 占位，待后续步骤实现。
...
- **`core/` → 静态库 `qharness_core`**：纯 C++17，**不依赖 Qt**（只用 vendored 的 nlohmann/json）。`core/CMakeLists.txt` 不链接任何 Qt 目标。
...
`engine/{Engine,EngineReActLoop}`、`provider/{Provider,ProviderOpenAI,ProviderClaude}`、`tool/Tool`、`robot/{Robot,RobotFeishu}`、`memory/{Memory,MemoryFile}`。基类纯虚（`virtual ... = 0`），派生类用内联 `/* TODO */` 体（保证 header-only 可独立编译）。`Provider::generate` 失败约定抛 `std::runtime_error`。`core/tests/skeleton_compile_check.cpp` 强制实例化各派生类做链接期检查。

### 命名空间

`qh::schema`、`qh::engine`、`qh::provider`、`qh::tool`、`qh::robot`、`qh::memory`、`qh::app`、`qh::test`。`core/` 目录是 `qharness_core` 的公共 include 根，内部头文件用根相对路径引用（如 `#include "schema/Message.h"`、`#include "provider/Provider.h"`）。
...
- **Anaconda Qt 干扰**：本机 PATH 上有 Anaconda 自带 Qt，会与项目所用 Qt 5.14.2 冲突。`app/CMakeLists.txt` 的 windeployqt post-build 把正确 Qt 的 `bin` 置于 PATH 最前再部署，**只能在 `build/out/<config>/` 运行 `qharness_app.exe`**（Qt DLL 已部署至此）；`build/core/`、`build/app/` 下的 exe 没有运行时 DLL，直接跑会报缺 Qt5Core.dll。
